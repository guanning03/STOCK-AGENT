...
def main(
    ...
    summary_llm: Optional[str] = typer.Option(None, help="Whether to call real LLM to generate natural language summary after backtest: true|false (use config if empty)"),
    ...
    offline: str = typer.Option("", help="Shortcut for --data-mode offline_only (pass 'true' or '1')"),
):
    ...
    try:
        if offline and str(offline).lower() not in ("", "false", "0", "no"):
    ...
    if summary_llm is not None:
        config.setdefault("backtest", {})["summary_llm"] = str(summary_llm).lower() not in ("false", "0", "no", "none")
